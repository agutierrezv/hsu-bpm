angular.module('myApp').controller('ListTramiteController', ['$http', '$scope', '$location', '$q', '$timeout', 'NavigationService', 'TramiteService', function($http, $scope, $location, $q, $timeout, NavigationService, TramiteService) {
	...
	$scope.add = function () {
		$location.path('/tramite/add');
	};

	$scope.edit = function (obj) {
		$location.path('/tramite/edit/' + obj._id);
	};
	$scope.view = function (obj) {
		$location.path('/tramite/' + obj._id);
	};
	$scope.delete = function (obj) {
		$location.path('/tramite/delete/' + obj._id);
	};
	...
	$scope.importData = function () {
		...
		$location.path('/import/tramite');		
	};

	$scope.exportAsCsv = function () { 
		$scope.dataReceived = false;	
		TramiteService.getFileAsCsv()
		.then(function(httpResponse) {
			sendFile(httpResponse, "tramites.csv", "text/csv");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXml = function () { 
		$scope.dataReceived = false;	
		TramiteService.getFileAsXml().then(function(httpResponse) {
			sendFile(httpResponse, "tramites.xml", "text/xml");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXlsx = function () {
		$scope.dataReceived = false;	
		TramiteService.getFileAsXlsx().then(function(httpResponse) {
			sendFile(httpResponse, "tramites.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	function sendFile(httpResponse, fileName, mime) {
		...
		var filename = headers["x-filename"] || fileName || "tramites.csv";
