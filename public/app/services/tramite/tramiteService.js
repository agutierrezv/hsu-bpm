angular.module('myApp').service('TramiteService', ['$http', '$q', 'UrlService', 'baseApi', function ($http, $q, UrlService, baseApi) {
	...
	// Getting the tramite API documentation URL and a promise to know when it's loaded.
	docUrlReceived = UrlService.tramiteUrl.then(function (url) {
		docUrl = url;
	});

	// When the documentation is received, getting the tramites resource URL and a promise to know when it's loaded.
...
//Process each property
		...
		clause = addStringLike('origin', searchText);

		if (clause != null) {
			clauses.push(clause);
		}

		clause = addStringLike('originId', searchText);
		...
		clause = addStringLike('assignedTo', searchText);
	...
	TramiteService.getCount =  safeCall(function (opts) {
		opts = opts || {};
		opts.count = true;		
	...
	TramiteService.getList = safeCall(function (opts) {
		opts = opts || {};
		var q = buildBaucisQuery(opts);
		return $http.get(resourceUrl + q);
	});

	TramiteService.getListAsCsv = safeCall(function () {
		return $http({
			method: 'GET', 
			url: resourceUrl, 
	...
	TramiteService.getFileAsCsv = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	TramiteService.getFileAsXml = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	TramiteService.getFileAsXlsx = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	TramiteService.getToEdit = safeCall(function (id) {
		return TramiteService.get(resourceUrl + '/' + id );
	});

	TramiteService.get = safeCall(function (link) {
		return $http.get(link);
	});

	TramiteService.add = safeCall(function (item) {
		return $http.post(resourceUrl, JSON.stringify(item));
	});

	TramiteService.update = function (item) {
		return $http.put(resourceUrl + '/' + item._id, JSON.stringify(item));
	};

	TramiteService.delete = safeCall(function (id) {
		return $http.delete(resourceUrl + '/' + id);
	});

	TramiteService.deleteMany = safeCall(function (ids) {
		var msg = { 
			'className' : 'tramite',
			'ids'		: ids
		};	
	...
	TramiteService.deleteAll = safeCall(function (opts) {
		var msg = { 
			'className' : 'tramite',
